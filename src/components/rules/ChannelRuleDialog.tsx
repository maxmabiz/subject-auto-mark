...
import { SUPPORTED_SEARCH_FIELDS, defaultFuzzy, isSearchFieldSupported } from "@/domain/matching/fieldMap";
...
const emptyCondition = (): RuleCondition => ({ searchField: "", keyword: "", fuzzy: true });
...
export function ChannelRuleDialog({
  ...
  useEffect(() => {
    ...
    const next = rule ? ruleConditions(rule).map((item) => ({
      ...item,
      fuzzy: typeof item.fuzzy === "boolean" ? item.fuzzy : defaultFuzzy(item.searchField),
    })) : [];
  ...
  const filled = conditions.map((item) => ({
  ...
  const canSubmit = Boolean(
    ...
    level2.trim() &&
    filled.length > 0 &&
    filled.every((item) => item.searchField && item.keyword && isSearchFieldSupported(item.searchField)),
  );
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-[min(840px,calc(100%-32px))]" title={rule ? "编辑平台规则" : "新增平台规则"}>
        <div className="max-h-[min(70vh,640px)] space-y-4 overflow-auto px-5 py-4">
          ...
          <div className="space-y-2">
            ...
            {conditions.map((item, index) => (
              <div key={index} className="grid grid-cols-[minmax(0,1.2fr)_minmax(0,1.2fr)_8.5rem_auto] items-end gap-2">
                <div className="space-y-1.5">
                ...
                <div className="space-y-1.5">
                  {index === 0 ? <Label>是否模糊匹配</Label> : null}
                  <Select
                    value={item.fuzzy === false ? "no" : "yes"}
                    onValueChange={(value) => updateCondition(index, { fuzzy: value !== "no" })}
                  >
          ...
          <p className="rounded-md bg-slate-50 px-3 py-2 text-xs leading-5 text-muted">
            匹配键为平台 + 账号 + 每一组检索字段与关键词。多组之间是或：任一组命中即命中本规则。是否模糊匹配为「是」时按包含匹配，「否」时按完全匹配。
          </p>
        </div>
