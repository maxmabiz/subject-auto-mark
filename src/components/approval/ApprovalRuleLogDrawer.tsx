...
import { APPROVAL_LOG_ACTION_LABEL } from "@/domain/approval/log";
import type { ApprovalRule, ApprovalRuleChange, ApprovalRuleLog } from "@/domain/types";
import { formatDateTime } from "@/lib/format";
import { useAppStore } from "@/store/AppStore";

export function ApprovalRuleLogDrawer({
  rule,
  open,
  onOpenChange,
}: {
  ...
  const { approvalLogsFor } = useAppStore();
  const logs = rule ? approvalLogsFor(rule.id) : [];
        ...
        {rule ? (
          <div className="flex-1 overflow-auto px-5 py-4">
            <div className="mb-5 text-sm text-slate-500">
              {rule.approvalName} / {rule.paymentType}
            </div>
            {logs.length ? (
              <ol className="space-y-5 border-l border-slate-200 pl-4">
                {logs.map((log) => (
                  <li key={log.id} className="relative">
                    ...
                    <div className="text-sm font-medium">{APPROVAL_LOG_ACTION_LABEL[log.action]}</div>
...
function LogDetail({ log }: { log: ApprovalRuleLog }) {
...
function visibleChanges(log: ApprovalRuleLog): ApprovalRuleChange[] {
  if (log.action === "create" || log.action === "import") {
    const created = log.changes.every((item) => item.from === "—");
    if (created) return log.changes.filter((item) => item.field === "科目");
  }
  return log.changes;
}
